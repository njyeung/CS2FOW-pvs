...
- CheckTransmit clears only the verified primary entity-send list, leaves full updates untouched, and lets visible entities return through ordinary snapshots.
...
- `cs2fow_entity` records fixed-size, aggregated evidence only for primary bits that CS2FOW actually cleared.
...
- BVH8 version 3 validates the complete layout and CRC before atomically replacing a bake; the C++ baker owns Valve package discovery and extraction.
